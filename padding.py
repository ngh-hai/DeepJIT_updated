...
def padding_length(line, max_length): # pad or truncate line to max_length
    line_length = len(line.split())
    if line_length < max_length:
        return str(line + ' <NULL>' * (max_length - line_length)).strip() # add <NULL> token if shorter than max_length
    elif line_length > max_length:
        line_split = line.split()
        return ' '.join([line_split[i] for i in range(max_length)]) # select first max_length token
    else:
        return line

def padding_data(data, dictionary, params, type):
    if type == 'msg': # pad message
        pad_msg = padding_message(data=data, max_length=params.msg_length) # 
        pad_msg = mapping_dict_msg(pad_msg=pad_msg, dict_msg=dictionary) # map token to code in dictionary
        return pad_msg
    elif type == 'code':
